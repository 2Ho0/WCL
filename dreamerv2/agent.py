...
class Agent(common.Module):
    ...
    def save_cl_params(self, filename, task_id, rep):
    ...
    def load_cl_params(self, filename):
        with pathlib.Path(filename).open('rb') as f:
            values = pickle.load(f)
        task_id, rep = values.values()
        return task_id, rep
...
class WorldModel(common.Module):
    ...
    def video_pred(self, data, key):
        ...
        truth = data[key][:6] + 0.5 # 6 samples are shown in the gif.
        embed = self.encoder(data)
